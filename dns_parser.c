...
void parse_response(char *buffer, int payload_len, binary_string_t *answer,
                    uint32_t *question_hash, uint32_t *client_hash) {
  ...
  if ((ntohs(dns_header->flags) & 0x8000) != 0) {
    ...
    *client_hash = fnv1a_hash_func(id_w_question);
    ...
    size_t answer_len =
        payload_len - (sizeof(struct dns_header) + question_len);
